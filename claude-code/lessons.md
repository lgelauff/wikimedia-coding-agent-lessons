...
Two commits landed on main and the installed copy on the server did not change. Diagnosis
[confirmed, 2026-09-23]: nothing on that host populates the tree — no timer, no cron, no
installer, no git remote; the trees are pushed by the desktop remote runtime when a connection
syncs. The per-plugin `.synced` marker is a **last-checked stamp, not a content hash**: all
seven plugin directories were stamped at the same instant that morning while their contents
still dated from ten days earlier. The most likely gate is the declared plugin version, which
the content commits did not bump — so the version silently acts as a cache key.

**Rules:** bump the plugin (and marketplace) version in the same commit as any skill or playbook
