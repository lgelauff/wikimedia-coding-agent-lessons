...
- [`latex/lessons.md`](latex/lessons.md) — building, verifying and syncing LaTeX: why "it compiled" is not evidence (LaTeX writes a PDF through errors), the four checks a build gate needs, BibTeX's non-zero exit on warnings vs `set -e`, `authblk` expanding `\author`, and Overleaf sync on the free tier (zips are not expanded, folder uploads nest and break `\input`, and download-and-diff is the only protection there is)
...
1. **Development** — building & shipping the software safely.
   Lessons: [toolforge](toolforge/lessons.md), [wikimedia](wikimedia/lessons.md), [flask](flask/lessons.md). Tooling: `pr-check`, `e2e-design` (choose the end-to-end journeys worth testing, build each so it asserts what a user perceives, and re-think a suite that grew by accretion), `browser-verify`, `session-close` (end-of-session protocol: secure at-risk work — including the gitignored files `git status` is structurally blind to, via `git_hygiene --ignored` — then evaluate what the session established and route actions into the project's queues), the dev guards (`block_ssh`, `block_secret_read`, `github_write_permission`, `webfetch_content_check`, `dev_stack_reminder`, `git_hygiene_session`), policies (`is_ssh_command`, `classify_github_op`), scripts (`llm_review`, `scope`, `secrets`, `git_hygiene`, `check_inline_js`, `post_pr_screenshots`), and `git-hooks/pre-commit`.

2. **Data analytics** — querying & measuring Wikimedia data.
   Lessons: [wikimedia-analytics](wikimedia-analytics/lessons.md) (MediaWiki API parsing, PAWS / PAWS-SQL, dumps, replica schema, bot-inflation, Phabricator), [matplotlib](matplotlib/lessons.md) (publication figures). Tooling: `wikimedia-enterprise` (auth + usage for the credentialed Enterprise API — On-demand/Snapshot/Realtime/Metadata), `overnight-run` (prep → rehearse → launch → report protocol for unattended 8–10 h runs; playbook + skill), `liftwing-llm` (playbook + `llm_provider.py`; Wikimedia-hosted open-weight LLMs via the keyless OpenAI-compatible endpoint — the free, WMF-hosted option for bulk classification/extraction; 100 req/h anon ceiling + Toolforge escalation). Otherwise still lesson-heavy and tooling-light — a known gap.

3. **Research & writing prep** — sourcing and curation that precede a writing effort.
   Lessons: [latex](latex/lessons.md) (build gates, toolchain traps, Overleaf sync). Skills: `source-connectors` (model every source you collect from as a declared connector — endpoint, auth, access policy, licence, retrieval recipe; be a good neighbour rather than a scraper) `latex-change-review` (show what a `.tex` edit actually changed — text diff for content, cropped before/after PNGs for styling) and `arxiv-submission` (prepare a LaTeX paper for arXiv and prove it is ready — the traps are the ones your checks pass: cm-super fonts that are Type 1, embedded and still visibly wobbly; a figure scoring a clean 1:1 while 67pt short of the measure; an `\IfFileExists` guard that prints "not present" into the published paper). Playbook: `research-data-collection` ("curate, don't RAG-dump"). Pipeline: `source_discovery` → `score_candidates` → `candidates_to_pending`. Guard: `block_zotero`. Utility: `check_wayback_coverage` (citation robustness).
