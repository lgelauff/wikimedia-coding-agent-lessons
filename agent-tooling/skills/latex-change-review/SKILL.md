...
`${AGENT_TOOLING_ROOT}/playbooks/latex-change-review.md`.
...
3. **Classify** the change:
   ```bash
   python3 "${AGENT_TOOLING_ROOT}/scripts/latex_classify.py" before/main.tex main.tex
   ```
   → `content` / `styling` / both. Mixed ⇒ do both views.
4. **Content view (no compile):**
   ```bash
   python3 "${AGENT_TOOLING_ROOT}/scripts/latex_diff_html.py" before/main.tex main.tex \
...
5. **Styling view (compile + image):**
   ```bash
   python3 "${AGENT_TOOLING_ROOT}/scripts/latex_visual_diff.py" \
