...
| `mcp_auth_reminder.py` | UserPromptSubmit | When a prompt links to a configured MCP server that still needs authenticating (a pasted Jam recording being the recurring case), injects the exact `claude mcp login <name>` command so the answer isn't a dead end. Only shells out when a URL actually matches a configured server, so ordinary prompts cost nothing. Fails open. |
