...
**Installed as part of the plugin, these wire automatically** via [`hooks.json`](hooks.json) (paths use `${CLAUDE_PLUGIN_ROOT}`). Wired by default: `block_ssh`, `memory_guard`, `github_write_permission`, `openrouter_permission` (PreToolUse), `git_hygiene_session` (SessionStart), `dev_stack_reminder` (UserPromptSubmit), and `webfetch_content_check` (PostToolUse). `block_zotero.py` ships as a file but is **not** wired (personal path block — wire it yourself if you want it).
