...
| **Adapters** | **product-specific** — the only Claude-coupled layer | `adapters/<agent>/` | Claude Code: `SKILL.md` files, PreToolUse/PostToolUse hook wrappers, `settings.json` snippets |
...
├── conventions.md           how to author each layer
├── scripts/                 agent-agnostic; tests in scripts/tests/
├── policies/                agent-agnostic decision executables; tests in policies/tests/
├── playbooks/               agent-agnostic procedures (markdown)
├── git-hooks/               standard git hooks
└── adapters/
    └── claude-code/
        ├── hooks/           thin: parse Claude event → call ../../policies/X → emit decision
        ├── skills/          SKILL.md referencing ../../playbooks/X + Claude orchestration
        └── settings/        allowlist + hook-wiring snippets
```

Only `adapters/claude-code/` exists today. Other agents are **not** scaffolded until needed (YAGNI) — the contract below is what makes adding one later cheap.

## Adapter contracts

So a future `adapters/<agent>/` can be added without touching the core:
...
- **Permissions:** never allowlist arbitrary code execution; prefer exact invocations or bundled script paths. (See `adapters/claude-code/settings/` and the allowlist method.)
...
| `claude-code/hooks/block_ssh.py`, `webfetch_content_check.py` | policy → `agent-tooling/policies/`; Claude wrapper → `adapters/claude-code/hooks/` | general policy + thin adapter |
| `claude-code/hooks/github_write_permission.sh`, `openrouter_permission.sh` | `adapters/claude-code/hooks/` (logic extractable to `policies/` if reused) | gating tied to the agent's permission model |
| `claude-code/allowlist.md`, `conventions.md` | `agent-tooling/` (conventions) + `adapters/claude-code/settings/` (allowlist) | split general rules from Claude wiring |
| `wiki-polis/.claude/skills/{pr-check,local-e2e,staging-chrome-test}` | playbook → `agent-tooling/playbooks/`; `SKILL.md` → `adapters/claude-code/skills/` | general procedure + Claude adapter |
