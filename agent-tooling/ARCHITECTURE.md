...
└── agent-tooling/                    the plugin (= the Claude Code adapter)
    ...
    ├── policies/        AGENT-AGNOSTIC — decision executables (planned; tests in policies/tests/)
...
| `claude-code/hooks/block_ssh.py`, `webfetch_content_check.py` | policy → `agent-tooling/policies/`; Claude wrapper → `agent-tooling/hooks/` | general policy + thin adapter |
